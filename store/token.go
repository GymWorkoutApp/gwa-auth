...
func NewMemoryTokenStore() (store TokenStore, err error) {
	store, err = NewFileTokenStore(":memory:")
	return
}

// NewFileTokenStore create a token store instance based on file
func NewFileTokenStore(filename string) (store TokenStore, err error) {
	db, err := buntdb.Open(filename)
	if err != nil {
		return
	}
	store = &TokenStoreMemory{db: db}
	return
}
...
func (ts *TokenStoreMemory) Create(info models.TokenInfo) (err error) {
	ct := time.Now()
...
// remove key
func (ts *TokenStoreMemory) remove(key string) (err error) {
...
func (ts *TokenStoreMemory) RemoveByCode(code string) (err error) {
	err = ts.remove(code)
	return
}

// RemoveByAccess use the access token to delete the token information
func (ts *TokenStoreMemory) RemoveByAccess(access string) (err error) {
	err = ts.remove(access)
	return
}

// RemoveByRefresh use the refresh token to delete the token information
func (ts *TokenStoreMemory) RemoveByRefresh(refresh string) (err error) {
	err = ts.remove(refresh)
	return
}

func (ts *TokenStoreMemory) getData(key string) (ti models.TokenInfo, err error) {
...
func (ts *TokenStoreMemory) getBasicID(key string) (basicID string, err error) {
	verr := ts.db.View(func(tx *buntdb.Tx) (err error) {
		v, err := tx.Get(key)
		if err != nil {
			return
...
func (ts *TokenStoreMemory) GetByCode(code string) (ti models.TokenInfo, err error) {
	ti, err = ts.getData(code)
	return
}

// GetByAccess use the access token for token information data
func (ts *TokenStoreMemory) GetByAccess(access string) (ti models.TokenInfo, err error) {
...
func (ts *TokenStoreMemory) GetByRefresh(refresh string) (ti models.TokenInfo, err error) {
